...
```
curl --location '%ELASTICSEARCH_HOST%/_aliases' \
...
```
curl --location '%ELASTICSEARCH_HOST%/employees-alias/_doc' \
--header 'Content-Type: application/json' \
--data '{
   "FirstName":"GABRIEL",
   "LastName":"MARTINEZ",
   ...
   "Address":"Alameda Urkijo 33, 1ºI, Bilbao, 48008",
   "Gender":"Male",
   "Age":33,
   "MaritalStatus":"Unmarried",
   "Interests":"Sports,Music,Readaing"
}'
```
...
```
curl --location '%ELASTICSEARCH_HOST%/employees-alias/_doc/NCyTkpABgw6_7scSMt6a'
```
...
```
curl --location --request DELETE %ELASTICSEARCH_HOST%/employees-alias/_doc/NCyTkpABgw6_7scSMt6a'
```
...
```
curl --location --request GET '%ELASTICSEARCH_HOST%/employees-alias/_search' \
--header 'Content-Type: application/json' \
--data '{
    "query": {
        "term": {
...
Crear indice:
```
curl --location --request PUT '%ELASTICSEARCH_HOST%/employees-v2' \
--header 'Content-Type: application/json' \
--data '{
   "mappings":{
      "properties":{
...
Añadir datos:
```
curl -XPUT "%ELASTICSEARCH_HOST%/_bulk" --data-binary @Employees_raw.json -H "Content-Type: application/json"
```

Añadir indice al alias
```
curl --location --request PUT '%ELASTICSEARCH_HOST%/employees-v2/_alias/employees-alias'
```

- 2) Realiza alguna de las consultas anteriores. ¿Qué observas?

Las operaciones se realizan sonbre ambos indices.
